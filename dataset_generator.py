...
class DatasetGenerator:
    ...
    def __init__(self, ds_path = "dataset", clean_ds_file = "dataset.json", content_file_pattern = "document_{index}.txt",
                 content_dir = "dataset/data", query_file = "data.json"):
    ...
    def load_json(self, path, file_name):
        ...
        return data, f"{file_name} loaded from {Path(self.DS_PATH / self.CLEAN_DS_FILE)}"
    ...
    def dataset_format_converter(self, CONTENT_FILE_PATTERN, CONTENT_DIR):
        ...
        if not Path(self.DS_PATH / self.CLEAN_DS_FILE).is_file():
            ...
            message2 = self.save_json(data)
            print(colored(message2, "yellow"))
            
            for index, case in enumerate(query):
                query_data[index]['sentence'] = tokenizer(case['sentence'])
            
            query = self.similar_tokens(query, unique_tokens)

            query, data= self.calculate_tfidf(data_list, query_data, word_map)
    ...
    def calculate_tfidf(self, data, query_data,  word_map):
        tfidf_vectorizer = TFIDFVectorizer(word_map)
        tfidf_results = tfidf_vectorizer.fit_transform(data)

        for i, doc_vectors in enumerate(tfidf_results):
            data[i]['vectors'] = doc_vectors
            sum_vec = {}
            for sentence in doc_vectors:
                sum_vec = self.sum_dicts(sum_vec, sentence)
            data[i]['tf_idf'] = sum_vec
        
        tfidf_query = tfidf_vectorizer.transform(query_data)
        

        for i, doc_vectors in enumerate(tfidf_query):
            query_data[i]['tf_idf'] = doc_vectors


    
        return  tfidf_query, data
    
    def similar_tokens(self, query, unique_tokens, threshold=0.8):
        for case in query:
            similar_tokens = []
            max_val = 0
            similarity_ratio = 0
            ...
            query[case['case_id']] = similar_tokens
